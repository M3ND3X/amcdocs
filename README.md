...
API default service port is `9981`. However, if running the desktop or standalone releases from the website, the port is randomized by default.

A REST API implemented in Go is available, see [Amolecoin REST API Client Godoc](https://godoc.org/github.com/amolecoin/amolecoin/src/api#Client).
...
* `PROMETHEUS` - This is the `/api/v2/metrics` method exposing in Prometheus text format the default metrics for Amolecoin node application
...
```sh
curl http://127.0.0.1:9981/api/v1/csrf
```
...
```sh
curl http://127.0.0.1:9981/api/v1/health
```
...
```sh
curl http://127.0.0.1:9981/api/v1/version
```
...
```sh
curl http://127.0.0.1:9981/api/v2/metrics
```
...
go_memstats_mspan_inuse_bytes 79982
...
```sh
curl http://127.0.0.1:9981/api/v1/balance?addrs=7cpQ7t3PZZXvjTst8G7Uvs7XH4LeM8fBPD,nu7eSpT6hr5P21uzw7bnbxm83B6ywSjHdq,vczdwAwVsyVoGyZCFBoRPyWU3Qq35Ln1XB
```
...
```json
{
    ...
    "addresses": {
        "vczdwAwVsyVoGyZCFBoRPyWU3Qq35Ln1XB": {
            "confirmed": {
                "coins": 0,
                "hours": 0
            },
...
```sh
curl http://127.0.0.1:9981/api/v1/outputs?addrs=6dkVxyKFbFKg9Vdg6HPg1UANLByYRqkrdY
```

or

```sh
curl http://127.0.0.1:9981/api/v1/outputs?hashes=7669ff7350d2c70a88093431a7b30d3e69dda2319dcb048aa80fa0d19e12ebe0
```
...
Parses and validates a Amolecoin address. Returns the address version in the response.
...
```sh
curl -X POST http://127.0.0.1:9981/api/v2/address/verify \
...
```sh
curl http://127.0.0.1:9981/api/v1/wallet?id=2017_11_25_e5fb.wlt
```
...
```sh
curl http://127.0.0.1:9981/api/v1/wallet/transactions?id=2017_11_25_e5fb.wlt
```
...
```sh
curl http://127.0.0.1:9981/api/v1/wallet/transactions?id=2017_11_25_e5fb.wlt&verbose=1
```
...
```sh
curl http://127.0.0.1:9981/api/v1/wallets
```
...
```sh
curl http://127.0.0.1:9981/api/v1/wallets/folderName
```
...
```json
{
    "address": "/Users/user/.amolecoin/wallets"
}
```
...
```sh
curl http://127.0.0.1:9981/api/v1/wallet/newSeed
```
...
```sh
curl -X POST http://127.0.0.1:9981/api/v2/wallet/seed/verify \
 -H 'Content-type: application/json' \
...
```sh
curl -X POST http://127.0.0.1:9981/api/v1/wallet/create \
...
```sh
curl -X POST http://127.0.0.1:9981/api/v1/wallet/newAddress \
...
```sh
curl -X POST http://127.0.0.1:9981/api/v1/wallet/update \
 -H 'Content-Type: application/x-www-form-urlencoded' \
 -d 'id=$id' \
 -d 'label=$label'
```
...
```sh
curl http://127.0.0.1:9981/api/v1/wallet/balance?id=2018_03_07_3088.wlt
```
...
```sh
curl -X POST http://127.0.0.1:9981/api/v1/wallet/transaction -H 'content-type: application/json' -d '{
    "hours_selection": {
        "type": "auto",
        "mode": "share",
        "share_factor": "0.5"
...
```json
{
    ...
    "encoded_transaction": ""
}
```
...
```sh
curl -X POST http://127.0.0.1:9981/api/v2/wallet/transaction/sign -H 'content-type: application/json' -d '{
    "wallet_id": "foo.wlt",
    "password": "password",
    "encoded_transaction": ""
}'
...
```sh
curl -X POST http://127.0.0.1:9981/api/v1/wallet/unload \
 -H 'Content-Type: x-www-form-urlencoded' \
 -d 'id=2017_05_09_d554.wlt'
```
...
```sh
curl -X POST http://127.0.0.1:9981/api/v1/wallet/encrypt \
 -H 'Content-Type: application/x-www-form-urlencoded' \
 -d 'id=test.wlt' \
 -d 'password=$password'
```
...
```sh
curl -X POST http://127.0.0.1:9981/api/v1/wallet/decrypt \
 -H 'Content-Type: application/x-www-form-urlencoded' \
 -d 'id=test.wlt' \
 -d 'password=$password'
```
...
```sh
curl -X POST http://127.0.0.1:9981/api/v1/wallet/seed \
 -H 'Content-type: application/x-www-form-urlencoded' \
 -d 'id=test.wlt' \
 -d 'password=$password'
```
...
```sh
curl http://127.0.0.1:9981/api/v1/pendingTxs
```
...
```sh
curl http://127.0.0.1:9981/api/v1/pendingTxs?verbose=1
```
...
```sh
curl -X POST http://127.0.0.1:9981/api/v2/transaction -H 'Content-Type: application/json' -d '{
    "hours_selection": {
        "type": "auto",
        "mode": "share",
        "share_factor": "0.5"
...
```sh
curl http://127.0.0.1:9981/api/v1/transaction?txid=a6446654829a4a844add9f181949d12f8291fdd2c0fcb22200361e90e814e2d3
```
...
```sh
curl http://127.0.0.1:9981/api/v1/transaction?txid=a6446654829a4a844add9f181949d12f8291fdd2c0fcb22200361e90e814e2d3&verbose=1
```
...
```sh
curl http://127.0.0.1:9981/api/v1/transaction?txid=a6446654829a4a844add9f181949d12f8291fdd2c0fcb22200361e90e814e2d3&encoded=1
```
...
```sh
curl http://127.0.0.1:9981/api/v1/rawtx?txid=a6446654829a4a844add9f181949d12f8291fdd2c0fcb22200361e90e814e2d3
```
...
See [`coin.Transaction.Serialize`](https://godoc.org/github.com/amolecoin/amolecoin/src/coin#Transaction.Serialize).
...
```sh
curl -X POST http://127.0.0.1:9981/api/v1/injectTransaction -H 'content-type: application/json' -d '{
...
```sh
curl http://127.0.0.1:9981/api/v1/transactions?addrs=7cpQ7t3PZZXvjTst8G7Uvs7XH4LeM8fBPD,6dkVxyKFbFKg9Vdg6HPg1UANLByYRqkrdY&confirmed=1
```

To get unconfirmed transactions for one or more addresses:

```sh
curl http://127.0.0.1:9981/api/v1/transactions?addrs=7cpQ7t3PZZXvjTst8G7Uvs7XH4LeM8fBPD,6dkVxyKFbFKg9Vdg6HPg1UANLByYRqkrdY&confirmed=0
```

To get both confirmed and unconfirmed transactions for one or more addresses:

```sh
curl http://127.0.0.1:9981/api/v1/transactions?addrs=7cpQ7t3PZZXvjTst8G7Uvs7XH4LeM8fBPD,6dkVxyKFbFKg9Vdg6HPg1UANLByYRqkrdY
```
...
```sh
curl http://127.0.0.1:9981/api/v1/transactions?addrs=7cpQ7t3PZZXvjTst8G7Uvs7XH4LeM8fBPD,2K6NuLBBapWndAssUtkxKfCtyjDQDHrEhhT&verbose=1
```
...
```sh
curl -X POST 'http://127.0.0.1:9981/api/v1/resendUnconfirmedTxns'
```
...
```sh
curl -X POST -H 'Content-Type: application/json' http://127.0.0.1:9981/api/v2/transaction/verify \
...
```sh
curl http://127.0.0.1:9981/api/v1/blockchain/metadata
```
...
```sh
curl http://127.0.0.1:9981/api/v1/blockchain/progress
```
...
```json
{
    ...
    "peers": [
        {
            "address": "35.157.164.126:9982",
            "height": 2760
        },
        {
            "address": "63.142.253.76:9982",
            "height": 2760
        },
    ]
}
...
```sh
curl http://127.0.0.1:9981/api/v1/block?hash=6eafd13ab6823223b714246b32c984b56e0043412950faf17defdbb2cbf3fe30
```

or

```sh
curl http://127.0.0.1:9981/api/v1/block?seq=2760
```
...
```sh
curl http://127.0.0.1:9981/api/v1/block?hash=6eafd13ab6823223b714246b32c984b56e0043412950faf17defdbb2cbf3fe30&verbose=1
```

or

```sh
curl http://127.0.0.1:9981/api/v1/block?seq=2760&verbose=1
```
...
```sh
curl http://127.0.0.1:9981/api/v1/blocks?start=101&end=102
```
...
```sh
curl http://127.0.0.1:9981/api/v1/blocks?start=101&end=102&verbose=1
```
...
```sh
curl http://127.0.0.1:9981/api/v1/blocks?seqs=3,5,7
```
...
```sh
curl http://127.0.0.1:9981/api/v1/last_blocks?num=2
```
...
```sh
curl http://127.0.0.1:9981/api/v1/last_blocks?num=2&verbose=1
```
...
```sh
curl http://127.0.0.1:9981/api/v1/uxout?uxid=8b64d9b058e10472b9457fd2d05a1d89cbbbd78ce1d97b16587d43379271bed1
```
...
```sh
curl http://127.0.0.1:9981/api/v1/address_uxouts?address=6dkVxyKFbFKg9Vdg6HPg1UANLByYRqkrdY
```
...
```sh
curl http://127.0.0.1:9981/api/v1/coinSupply
```
...
```sh
curl "http://127.0.0.1:9981/api/v1/richlist?n=4&include-distribution=true"
```
...
```sh
curl "http://127.0.0.1:9981/api/v1/addresscount"
```
...
```sh
curl 'http://127.0.0.1:9981/api/v1/network/connection?addr=176.9.84.75:9982'
```
...
```sh
curl 'http://127.0.0.1:9981/api/v1/network/connections'
```
...
```sh
curl 'http://127.0.0.1:9981/api/v1/network/defaultConnections'
```
...
```sh
curl 'http://127.0.0.1:9981/api/v1/network/connections/trust'
```
...
```sh
curl 'http://127.0.0.1:9981/api/v1/network/connections/exchange'
```
...
```sh
curl -X POST 'http://127.0.0.1:9981/api/v1/network/connection/disconnect?id=999'
```
...
You must use a string for `"coins"` instead of an integer measured in "droplets" (the smallest unit of currency in Amolecoin, 1/1000000 of a amolecoin).
For example, if you sent 1 Amolecoin with `/api/v1/spend` you would have specified the `coins` field as `1000000`.
Now, you would specify it as `"1"`.

Some examples:

* 123.456 coins: before `123459982`, now `"123.456"`
